...
setPropertyValuesForSelect = ($select, event, property) ->
    $select.MPSelect()
    MP.api.propertyValues(event, property).done (results) ->
...
setSuperPropertyValuesForSelect = ($select, property) ->
    ...
    MP.api.query '/api/2.0/events/properties/values', params, null, (data) ->
        ...
        options = _.chain data
            .map (option) ->
                {
                    label: transformPropertyValueLabel property, option#if property == 'mp_country_code' then getCountryCodeForCodeelse if option.indexOf('@') < 0 and option.indexOf('http:') < 0 then option.toUpperCase() else option
                    value: option
                }
            .sortBy (option) ->
